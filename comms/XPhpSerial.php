 ...
 * Added function confPortEcho for linux to turn off/on echo for the serial port.
...
class PhpSerial
{
    ...
    public function PhpSerial()
    {
        ...
        // used this to find out what group php is in 'www-data'
        // echo exec('whoami');
        ...
        if (substr($sysName, 0, 5) === "Linux") {
            ...
            if ($this->_exec("stty --version") === 0) {
                echo 'GGG register shutdown <br>';
                register_shutdown_function(array($this, "deviceClose"));
            } else {
                    echo 'GGG Error down <br>';
                trigger_error(
    ...
    {
        echo 'GGG closing device 192 <br>';
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
            return true;
        }
    ...
    /**
     * Turn off echo for the port
     *
     * @param  string $mode Set the port echo mode. Availible modes :
     *                      -> "off" : no echo 
     *                      -> "on"  : echo on the port
     * @return bool
     */
    public function confPortEcho($mode)
    {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            trigger_error("Unable to set flow control mode : the device is " .
                          "either not set or opened", E_USER_WARNING);
        ...
        if ($ret === 0) {
            ...
            trigger_error(
                "Unable to control port echo : " . $out[1],
                E_USER_ERROR
            );
